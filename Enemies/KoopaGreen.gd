...
func reset_visually():
	...
	is_squished = false;
	direction = 1;
	$KoopaGSprite.flip_h = false;
	$KoopaGSprite.set_animation("walk");
	pass
...
func dead():
	...
	is_dead = true;
	$KoopaGSprite.set_animation("dead");
...
func got_hit_from_block(hitter_player = null):
	...
	$SquishedTimer.stop();
	$SquishedTimer.start();
	$KoopaGSprite.set_animation("squished");
	is_squished = true;
	is_kicked = false;
	hit_by_block = true;
	pass
...
func kick(right, body, playSound = true):
	...
	$KoopaGSprite.set_animation("kicked");
	$WakeUpTimer.stop();
	$SquishedTimer.stop();
	is_kicked = true;
	is_squished = false;
...
func _on_SquishedTimer_timeout():
	$SquishedTimer.stop();
	$WakeUpTimer.start();
	$KoopaGSprite.set_animation("wakeUp");
	pass
